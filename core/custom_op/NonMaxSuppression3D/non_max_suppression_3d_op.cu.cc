...
#include "non_max_suppression_3d_op.h"
...
class NonMaxSuppressionGPUOp : public OpKernel {
 public:
  explicit NonMaxSuppressionGPUOp(OpKernelConstruction* context)
      : OpKernel(context) {}
...
REGISTER_KERNEL_BUILDER(Name("NonMaxSuppression")
                            .TypeConstraint<float>("T")
                            .Device(DEVICE_GPU)
                            .HostMemory("iou_threshold")
                            .HostMemory("max_output_size"),
                        NonMaxSuppressionGPUOp);
